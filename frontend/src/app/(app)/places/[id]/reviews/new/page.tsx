...
export default function NewReviewPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  return (
    <div className="max-w-lg space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-xl font-bold">Tulis Review</h1>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="space-y-1">
          ...
          <Textarea {...register("description")} placeholder="Ceritakan pengalamanmu..." rows={4} />
        ...
        <Button type="submit" className="w-full" disabled={createReview.isPending}>
          {createReview.isPending ? "Menyimpan..." : "Simpan Review"}
        </Button>
      </form>
    </div>
  );
